...
def _report_store_uses_mongomock(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    against `RUNNER_MONGO_URI`/`MONGO_URI` (falling back to
    `localhost:27017`) on first use via `_build_mongo_client`, the same seam
    `derived.py`/`career_store.py`/`jobs.py` use. Without this fixture,
    `run_analysis`, `discover_player_builds`/`discover_reports`, and every
    api-ui route that reads report data would try a real network connection
    and hang. One fresh client per test (function-scoped fixture) mirrors the
    other stores' isolation above -- tests that need to seed/inspect report
    data directly call `open_report_store()` (sharing this fixture's client)
    or construct their own `ReportStore`/`mongomock.MongoClient()`.
    """
    client = mongomock.MongoClient()
