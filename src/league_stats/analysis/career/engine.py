...
is generated by re-ranking the track pool against the build's *current* category
weaknesses -- never against an order decided when the ladder started.

The ladder always shows three blocks. Which tracks fill them is driven by the
coach's significance signals, but a track that is not currently flagged is still
offered as a stretch goal rather than leaving a slot empty.
"""
...
def _fill_empty_slots(
    ...
    """Top the ladder back up to three blocks.

    Runs every report, not just the first: a build whose peer comparison had not
    landed yet, or whose pool was briefly thin, would otherwise stay short of
    three blocks forever.
    """
