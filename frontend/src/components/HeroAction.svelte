...
<style>
  ...
  .hero-action:hover { border-color: var(--color-accent); background: rgba(65, 183, 140, 0.08); transform: translateX(2px); }
  .hero-action-index {
    flex-shrink: 0; width: 22px; height: 22px; border-radius: 50%;
    display: inline-flex; align-items: center; justify-content: center;
    font-size: 11px; font-weight: 700; background: var(--color-surface); color: var(--color-neutral-400);
  }
  /* :global -- .hero-action-empty is the empty-state fallback rendered by Overview.svelte
     as a sibling of this component, not by HeroAction.svelte itself. */
